...
- **TBD-08 résolu — pas d'images produit dans YME :** YME ne stocke pas d'images. La table `media` reste présente pour un usage futur (bandeaux, documents), mais aucun flux d'images produit ne viendra de YME. Le frontend affiche un placeholder générique pour les produits sans image.
- **Conditionnement résolu — pas de contrainte de conditionnement :** YME n'a pas de notion de quantité minimale de commande ni de multiple. Le code `PACKAGING_VIOLATION` reste dans l'enum pour cohérence du catalogue d'erreurs mais ne sera jamais déclenché. Aucune logique de validation de conditionnement à implémenter.
- **Tarif client résolu — pas de catégorie tarifaire rattachée au compte :** Il n'y a pas de "tarif client" ou de "catégorie tarifaire" dans YME. La tarification est portée directement par l'article (prix catalogue) et par les éventuelles promotions plateforme. TBD-10 (cumul remises) simplifié : une seule source de prix par article, pas de multi-niveaux. À confirmer lors de la session YME si des remises article existent.
- **`customer_source_ref` stable :** Le code client YME ne change pas dans le temps. Pas besoin de mécanisme de migration de référence.
